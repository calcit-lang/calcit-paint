...
- 业务解析边界直接使用 `EdnMapView`，避免向函数签名暴露带 interior-mutability variant 的 `HashMap<Edn, Edn>` key 类型。
...
- Use `EdnMapView` directly at domain-decoding boundaries instead of exposing `HashMap<Edn, Edn>` keys that can contain interior-mutable EDN variants.
